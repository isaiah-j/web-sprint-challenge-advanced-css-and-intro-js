...
function addArtist(arr) {
	let artistInfo = {};
	artistInfo['id'] = artists.length;
	artistInfo['name'] = arr[0];
	artistInfo['years'] = arr[1];
	artistInfo['genre'] = arr[2];
	artistInfo['Nationality'] = arr[3];
	artistInfo['bio'] = arr[4];
...
function lotsOfArt(arr) {
	let over100Paintings = [];
	for (let i = 0; i < arr.length; i++) {
		let name = arr[i].name;
		let paintings = arr[i].paintings;
		let isOver100 = paintings > 100;
		if (isOver100) {
